...
def main():
    ...
    if args.test:
        ...
        test_loader = DataLoader(dataset(test_input, test_output, best_params['time_sequence'],test_trial_sizes), batch_size=best_params['batch_size'], shuffle=False)
        model = ThreeLayerLSTM(input_size, hidden_size, num_layers, best_params['dropout']).to(device)
        best_model_state_dict = torch.load('best_model_save/' + args.model_file)
        model.load_state_dict(best_model_file)
        ...
        print(f"\nTest each Loss_X: {test_each_loss_1:.4f}")
        print(f"\nTest each Loss_Y: {test_each_loss_2:.4f}")
        print(f"\nTest each Loss_O: {test_each_loss_3:.4f}")
        print(f"\nTest L2: {l2_test:.4f}")
